---
name: dev-team
description: Orchestrates a 6-role artifact-first development pipeline (BA → TechLead → Dev → BA Review → Tester → optional Builder). From a one-line task, produces spec.md, architecture.md, source code under code/, tests under tests/, QA report, and optionally a .exe via PyInstaller. Each stage has its own subagent and writes disk artifacts.
---
...
Orchestrate a 5-role artifact-first development pipeline using project subagents:
...
When this skill is invoked, you (the orchestrator) must:
...
The plugin ships seed memory templates under its own `skills/dev-team/memory-templates/` directory. On first run in a workspace, copy them to the workspace-local `.dev-team/memory/` so agents can read and append lessons.
